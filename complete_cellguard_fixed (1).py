...
def analyze_cell_balance(df):
    ...
    df = df.copy()
    cell_cols = [c for c in df.columns if re.match(r'^Cell\d+
...
with st.sidebar:
    ...
    connection = st.radio(
        "Data Source",
        ["📡 Live Vehicle BMS", "📁 Upload CSV File", "🧪 Demo Mode"],
        index=2
    )
    ...
    if "Live" in connection:
    ...
    elif "Upload" in connection:
        st.session_state.connection_status = 'offline'
        st.markdown('<div class="connection-offline">📁 FILE UPLOAD MODE</div>', unsafe_allow_html=True)
        
        uploaded_file = st.file_uploader(
            "Upload Battery BMS CSV",
            type=['csv'],
            help="Upload your battery management system CSV file"
        )
        ...
        if uploaded_file is not None:
            try:
                ...
                st.session_state.battery_data = df_uploaded
                st.info("💡 Data loaded! Switch to Demo Mode to see analysis.")
                
            except Exception as e:
                st.error(f"❌ Error loading file: {str(e)}")
                st.info("📋 Expected columns: Date, Time, voltage/Pack Vol, current/Curent, soc/Soc, temperature, Cell1-Cell24, Temp1-Temp4")
    else:
        st.session_state.connection_status = 'offline'
...
# Process data - ensure column name consistency
df_raw = st.session_state.battery_data.copy()

# Standardize column names if uploaded CSV has different names
column_mapping = {
    'Pack Vol': 'voltage',
...
numeric_cols = ['voltage', 'current', 'soc', 'temperature']
for col in numeric_cols:
    if col in df_raw.columns:
        df_raw[col] = pd.to_numeric(df_raw[col], errors='coerce')

# Convert cell columns to numeric
cell_cols = [c for c in df_raw.columns if re.match(r'^Cell\d+
...
# Technician Mode
    ...
    with col3:
        if st.button("📄 Download Sample CSV", use_container_width=True):
            sample_csv = gen_sample_data_shell(n=50).to_csv(index=False)
            st.download_button(
                label="⬇️ Download Sample",
                data=sample_csv,
                file_name="sample_bms_data.csv",
                mime="text/csv",
                use_container_width=True
            )
...
if 'time' not in df_raw.columns:
    df_raw['time'] = range(len(df_raw))

# Add Cycle column if missing
if 'Cycle' not in df_raw.columns:
    df_raw['Cycle'] = df_raw['time'] // 2
df_feat = make_features(df_raw, window=window)
df_feat = analyze_cell_balance(df_feat)
df_out = run_models(df_feat, contamination=contamination)
...
# Convert temp sensor columns to numeric
temp_cols = [c for c in df_raw.columns if re.match(r'^Temp\d+
df_feat = make_features(df_raw, window=window)
df_feat = analyze_cell_balance(df_feat)
df_out = run_models(df_feat, contamination=contamination)
...
# If still have NaN in critical columns, use default values
if 'voltage' in df_raw.columns:
    df_raw['voltage'] = df_raw['voltage'].fillna(78.4)
if 'current' in df_raw.columns:
    df_raw['current'] = df_raw['current'].fillna(2.5)
if 'soc' in df_raw.columns:
    df_raw['soc'] = df_raw['soc'].fillna(50)
if 'temperature' in df_raw.columns:
    df_raw['temperature'] = df_raw['temperature'].fillna(32)
df_feat = make_features(df_raw, window=window)
df_feat = analyze_cell_balance(df_feat)
df_out = run_models(df_feat, contamination=contamination)
